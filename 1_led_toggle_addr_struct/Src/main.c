...
// these are structs that map over the physical addresses. they are instead used to access the members of each register
typedef struct {
	volatile uint32_t MODER;
	volatile uint32_t DUMMY[4]; // dummy arrays used because there are gaps in memory addresses between the registers we actually want and the registers we want to ignore
